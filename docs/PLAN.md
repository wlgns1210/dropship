...
(이름은 Skiff 로 바꿨다 — Dropship 은 이미 삼성이 쓰는 제품명이다.)
...
MVP에서 **제외**(Phase 5로 이월): 비밀번호 보호, 다운로드 횟수 제한, 이미지·영상 미리보기, 다국어.
(ZIP 일괄 다운로드는 파일 개수 상한을 100개로 올리면서 먼저 구현했다.)
...
삼성 Dropship 과 동일한 `단어/숫자` 패턴을 쓴다: `<호스트>/oslo/113245`
...
1. **비밀번호 보호** — 공유 코드만으로는 암호학적으로 안전하지 않다. Phase 5 의 핵심 항목이고, 지금 남아 있는 가장 큰 보안 공백이다.
2. **백업** — SQLite 와 업로드 파일이 볼륨 하나에 있고 백업이 없다. 보관 기간이 최대 7일이라 손실 범위는 제한적이지만 전략 자체는 없다.
...
- ~~파일 크기 상한~~ → **전송당 합계 1GB.**
- ~~언어~~ → **한국어.**
- ~~도메인~~ → **사지 않는다.** 학교 VM 의 `service.gsmsv.site` 를 쓴다.
- ~~AWS 계정 상태~~ → 기존 계정 · 서울(ap-northeast-2).
- ~~서비스 성격~~ → **포트폴리오.** 다만 실사용 서비스와 유사하게 운영한다.
- ~~인프라~~ → 서버리스로 만들었다가 **단일 VM + 컨테이너**로 옮겼다. 두 구현을 인터페이스 뒤에 모두 남겨 `DEPLOY_MODE` 로 고른다.
